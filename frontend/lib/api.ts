...
class APIService {
    ...
    async getResumeAnalysisById(id: string) {
        return this.request(`/api/resume/analysis/${id}`);
    }
    ...
    // AI
    async generateRoadmap(targetRole?: string, jobId?: string) {
        return this.request('/api/ai/roadmap', {
            method: 'POST',
            body: JSON.stringify({ target_role: targetRole, job_id: jobId }),
        });
    }
    ...
    async getRoadmapHistory() {
        return this.request('/api/ai/roadmaps/history');
    }

    async getRoadmapById(id: string) {
        return this.request(`/api/ai/roadmap/${id}`);
    }
