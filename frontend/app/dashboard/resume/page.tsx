...
import { AlertCircle, Map as MapIcon, RotateCcw, Sparkles, Check, Lightbulb, Navigation, Loader2 } from 'lucide-react';
...
export default function ResumePage() {
    ...
    useEffect(() => {
        const loadExistingAnalysis = async () => {
            try {
                const res = await api.getResumeAnalysis() as Record<string, unknown>;
                if (res && res.analysis) {
                    setResult(res);
                }
            } catch (err) {
                // Ignore, likely 404 meaning no existing resume
            } finally {
                setLoading(false);
            }
        };
    ...
    // Normalize data shape between /upload (flat) and /analysis (nested {resume, analysis})
    const analysis = (result?.analysis || result) as Record<string, unknown> | undefined;
    const activeResume = (result?.resume || result) as Record<string, unknown> | undefined;
        ...
        <div className="max-w-[1000px] mx-auto space-y-8 animate-[fade-in_0.6s_ease-out_forwards]">
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-32 text-slate-500">
                    ...
                    <p className="text-sm font-medium">Retrieving your intelligence briefing...</p>
                </div>
            ) : (
                <>
                    {!result && (
                        ...
                        <>
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 pb-2">
                                <div>
                                    ...
                                    {(activeResume as any)?.file_url && (
                                        <p className="text-sm text-slate-500 mt-2 flex items-center gap-2">
                                            <Check className="w-4 h-4 text-emerald-500" />
                                            Active Resume: <a href={(activeResume as any).file_url as string} target="_blank" rel="noopener noreferrer" className="text-sky-600 hover:underline">View Source Document</a>
                                        </p>
                                    )}
                                </div>
                                <button
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                ...
                                <div className="lg:col-span-7 flex flex-col gap-6">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white rounded-3xl border border-slate-100 p-6 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.02)]">
                                        {[
                                            { label: 'Skills Extracted', value: (activeResume?.skills as unknown[])?.length || 0 },
                                            { label: 'Experience Tier', value: `${activeResume?.experience_years as number || 0}y` },
                                            { label: 'ATS Compliance', value: `${analysis?.keyword_optimization as number || 0}%` },
                                    ...
                                    <div className="bg-white rounded-3xl border border-slate-100 p-8 flex-1 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.02)] flex flex-col justify-center">
                                        ...
                                        <div className="flex flex-wrap gap-2.5">
                                            ...
                                            {(activeResume?.skills as string[])?.slice(0, 5).map((skill: string) => (
                                            ...
                                            {(activeResume?.skills as string[])?.slice(5).map((skill: string) => (
